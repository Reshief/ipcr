...
void readConfigFile(const std::string sConfigFileName)
{
	...
	if (f.is_open())
	{
		...
		{
			f >> sKey >> sValue;
			if (sKey==SxMin){ g_fSxMin=atof(sValue.data());}
			if (sKey==SxMax){ g_fSxMax=atof(sValue.data());}
			if (sKey==SyMin){ g_fSyMin=atof(sValue.data());}
			if (sKey==SyMax){ g_fSyMax=atof(sValue.data());}
			if (sKey==Scaling){ g_fScaling=atof(sValue.data());}
		}
		f.close();
	} else {
